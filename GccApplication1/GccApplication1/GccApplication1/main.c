...
#define  esp8266_read    gets2

//-------------------
#define  wdt_reset()    #asm("wdr")
//Ensures the system doesn't hang indefinitely while waiting for ESP8266 responses or other tasks.
//Forces the system to reboot if something goes wrong, such as a communication error or unresponsive peripheral.
//------------------
...
//Sends an AT command to the ESP8266 and waits for a response within the timeout. and Returns the response string.
uchar *AtCommand(uchar *command, const int timeout, int debug);

//Sends data (like HTTP payload) to the ESP8266 and waits for confirmation.
uchar *AtData(uchar *data, const int timeout, int debug); 

void CIPData(uchar *connectionId, uchar *data); //AtCommand, handles direct data.
...
//------------------------------------
//Timer0 overflow ISR updates millis every time the timer overflows.
...
//--USART-----------------------------------
//Checks if data is available from the ESP8266 by reading the USART status register
...
//------------------------------------------
// ==> Servo Initialization Function  ==>code update
void Servo_Init() {
    // Configure Timer1 for Fast PWM mode, 50 Hz frequency
    TCCR1A = (1 << WGM11) | (1 << COM1A1); // Fast PWM, non-inverting mode on OC1A
    TCCR1B = (1 << WGM13) | (1 << WGM12) | (1 << CS11); // Prescaler = 8, Fast PWM with ICR1

    // Set the top value for 50 Hz frequency (16 MHz clock, prescaler = 8)
    ICR1 = 39999; // 16 MHz / (50 Hz * 8 prescaler) = 40000 - 1

    // Set initial pulse width for neutral position (1.5 ms, 90° position)
    OCR1A = 3000; // 1.5 ms / 20 ms * 39999 = 3000

    // Configure the output pin (e.g., PB1/OC1A) as output
    DDRB |= (1 << PB1);
}

//==>Servo Position Control Function
void Set_Servo_Position(uint angle) {
    // Map angle (0°-180°) to pulse width (1 ms - 2 ms)
    uint pulseWidth = 1000 + ((angle * 1000) / 180); // Convert angle to pulse width in microseconds
    OCR1A = (pulseWidth * 39999) / 20000; // Scale to ICR1 value
}
...
/*Ports (input/output).
  Timer0 for 1ms intervals.
  USART for communication with ESP8266.
